...
inst_file = ["instructions_ratings.txt"] # name of instructions files (needs to be .txt)
...
for cue in cue_dict:
    for accuracy in accuracies:
        thisExp.addData('start.time', globalClock.getTime()) # add trial onset time to the data file
        
        # update component parameters for each repeat
        Choice_Resp = event.BuilderKeyResponse()

        thisExp.addData('trialtype', cue) # add cue info to the data file
        thisExp.addData('trialaccuracy', accuracy)

        Cue.edges = cue_dict[cue]
        CueLabel.text = "{}\n({}% accuracy)".format(cue, accuracy)
    
        # ------Prepare to start Routine "ArousalRating"-------     
        ...
        ArousalComponents = [Cue, CueLabel, RatingScale, RatingInstr]
        for thisComponent in ArousalComponents:
            if hasattr(thisComponent, 'status'):
                thisComponent.status = NOT_STARTED
        
        # -------Start Routine "ArousalRating"-------
        while continueRoutine and RatingScale.noResponse:
        ...
        thisExp.addData('ArousalRating', RatingScale.getRating())

        # -------Ending Routine "ArousalRating"-------
        for thisComponent in ArousalComponents:
        ...
        # keep track of which components have finished
        ValenceComponents = [Cue, CueLabel, RatingScale, RatingInstr]
        ...
        # Add rating to experiment data
        thisExp.addData('ValenceRating', RatingScale.getRating())
    ...
    while continueRoutine:
        ...
        if fix.status == STARTED and t >= win.monitorFramePeriod * 0.75:
