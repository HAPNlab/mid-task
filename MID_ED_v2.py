...
cue_dict = {"+$5": 128, "-$5": 4, "$0": 6} # assign cue shapes (circle, square, hexagon) to cue types. Note: the order here is meaningful; trialtypes 1-3 correspond to the first dict item, trialtypes 4-6 correspond to the 2nd dict item, etc.
accuracies = [80, 50, 20] # desired accuracy levels (high, medium, low). Note: the order here is meaningful; e.g., the high accuracy level corresponds to trialtypes 1, 4, 7.
...
while trial_counter < len(stimuli):
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if t >= 0.0 and Cue.status == NOT_STARTED:
            ...
            Cue.setAutoDraw(True); CueLabel.setAutoDraw(True)
        ...
        if Cue.status == STARTED and t >= frameRemains:
            Cue.setAutoDraw(False); CueLabel.setAutoDraw(False)
